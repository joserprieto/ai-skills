...
description: >-
  ...
  documents, design systems, or UI components. Also use when user says "Avincis colors",
  "Avincis brand", "corporate design", "brand tokens", "Avincis diagram", "Avincis styling",
  "corporate presentation", or "brand guidelines".
license: MIT
metadata:
...
Standardized color mapping for all diagram elements. Apply these tokens to every draw.io,
PlantUML, Mermaid, or SVG diagram.
...
**Global font:** `fontFamily=TrebuchetMS` (no space) on ALL elements. Draw.io requires
the font name without space. Labels use `fontColor=#242846` (blue.800) unless a semantic color
applies.
